...
function PackageList() {
  ...
  const loadPackages = useCallback(async (destinationId = "") => {
    ...
    setError("");
    try {
      const packageResult = (availableOnly || !isAuthenticated)
        ? await packageService.available("", destinationId)
        : await packageService.list(destinationId ? "" : "");
      ...
      <section className="package-grid">
        {packages.map((travelPackage) => (
          <article className="package-card" key={travelPackage.packageId}>
            {travelPackage.thumbnailUrl && <div className="package-card-image"><img src={travelPackage.thumbnailUrl} alt={`${travelPackage.packageName} destination`} onError={(event) => { event.currentTarget.parentElement.style.display = "none"; }} /></div>}
